...
impl<K: Eq + Hash + Clone, T: Group + Clone> Tree<K, T> {
    ...
    pub fn set_transform(&mut self, transform: T, from_key: K, to_key: K) {
        ...
        if self.transforms.contains_key(&from_to_key) {
            ...
            let mut has_to_key = self.parents.contains_key(&to_key);

            if has_from_key && has_to_key {
                let from_root = self.get_root(from_key.clone());
                let to_root_path = self.get_ancestry(to_key.clone());
                if to_root_path.last() == Some(&from_root) {
                    panic!("Adding link would cause a cycle");
                }

                // Make `to_key` the root of it's tree so we can connect to it
                for pair in to_root_path.windows(2).rev() {
                    let (from_key, to_key) = (pair[0].clone(), pair[1].clone());
                    self.parents.remove(&from_key);
                    self.parents.insert(to_key, from_key);
                }

                assert!(self.parents.get(&to_key).is_none()); // It should be root now
                has_to_key = false; // We made sure of this
            }
...
impl<K: ToString, T> Tree<K, T> {
    pub fn output_dot<S: ToString>(&self, title: S) -> String {
        let escape = |s: &K| s.to_string().replace(r#"""#, r#"\""#);
        let content = self
            .parents
            .iter()
            .map(|(to_key, from_key)| {
                format!("  \"{}\" -- \"{}\"", escape(from_key), escape(to_key))
            })
            .collect::<Vec<_>>()
            .join("\n");
        format!(
